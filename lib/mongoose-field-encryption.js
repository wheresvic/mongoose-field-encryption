...
const encrypt = function (clearText, secret, saltGenerator, encryptOptions = {}) {
...
const decrypt = function (encryptedHex, secret, decryptOptions = {}) {
  if (encryptedHex === undefined || encryptedHex === null) {
    return encryptedHex;
  }
  let clearValue = '';
  try {
    ...
    if (encryptedArray.length === 1) {
      clearValue = decryptAes256Ctr(encryptedArray[0], secret);
    }
    ...
    clearValue = Buffer.concat([decrypted, decipher.final()]).toString();
  }
  catch (error) {
    if (decryptOptions.notifyDecryptFails) {
      throw error;
    }
    // don't print error, might flood logs
  }
  return clearValue;
};
...
const fieldEncryption = function (schema, options) {
  ...
  // Added option for a user to be able to encrypt each field of an array, rather than encrypting the 
  // entire array as a whole object.
  // defaults to false in order to maintain backward compitability. 
  const encryptEachArrayItem = options.encryptEachArrayItem !== undefined ? options.encryptEachArrayItem : false;
  ...
  for (const field of fieldsToEncrypt) {
    ...
    schemaField[encryptedFieldData] = { type: Schema.Types.Mixed };
    ...
    for (const field of fields) {
      ...
      if (!obj[encryptedFieldName] && typeof fieldValue !== "undefined") {
        ...
        if (typeof fieldValue === "string") {
          ...
          let value;
          if (Array.isArray(fieldValue) && encryptEachArrayItem) {
            value = [];
            for (let i = 0; i < fieldValue.length; i++) {
              value.push(encryptionStrategy(JSON.stringify(fieldValue[i]), secret, saltGenerator));
            }
            //Object.defineProperty(obj, encryptedFieldData, value, {writable: true, enumerable: true} );
          } else {
            value = encryptionStrategy(JSON.stringify(fieldValue), secret, saltGenerator);
          }
    ...
    for (const field of fields) {
      ...
      if (obj[encryptedFieldName]) {
        if (obj[encryptedFieldData]) {
          ...
          if (Array.isArray(encryptedValue) && encryptEachArrayItem) {
            obj[field] = [];
            for (let i = 0; i < encryptedValue.length; i++) {
              obj[field].push(JSON.parse(decrypt(encryptedValue[i], secret, { notifyDecryptFails: notifyDecryptFails })));
            }
          } else {
            obj[field] = JSON.parse(decrypt(encryptedValue, secret, { notifyDecryptFails: notifyDecryptFails }));
          }
