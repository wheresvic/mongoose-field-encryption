...
const encryptAes256Ctr = function (text, secret) {
  const cipher = crypto.createCipheriv(deprecatedAlgorithm, secret);
...
const decryptAes256Ctr = function (encryptedHex, secret) {
  const decipher = crypto.createDecipheriv(deprecatedAlgorithm, secret);
...
const encrypt = function (clearText, secret, saltGenerator) {
  ...
  const cipher = crypto.createCipheriviv(algorithm, secret, iv);
...
const decrypt = function (encryptedHex, secret) {
  ...
  const decipher = crypto.createDecipheriviv(algorithm, secret, iv);
